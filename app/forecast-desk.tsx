...
import { Activity, ArrowUpRight, CalendarDays, ChevronRight, CircleAlert, CircleCheck, Clock3, RefreshCw, Radio, ShieldCheck, SlidersHorizontal } from 'lucide-react';
...
type Forecast = { week: number; games: Game[]; retrievedAt: string; source: string; model: { label: string; ratings: Record<string, number>; notes: string } };
...
function probabilityFor(game: Game, ratings: Record<string, number>, adjustments: Record<string, number>) {
  const home = (ratings[game.home] ?? 0) + (adjustments[game.home] ?? 0);
  const away = (ratings[game.away] ?? 0) + (adjustments[game.away] ?? 0);
  return 1 / (1 + Math.exp(-(home - away + (game.neutral ? 0 : 1.1)) / 4.8));
}
...
export function ForecastDesk() {
  ...
  const renderedGames = useMemo(() => (forecast?.games ?? []).map((game) => {
    const adjustedHome = probabilityFor(game, forecast?.model.ratings ?? {}, live?.adjustments ?? {});
  ...
  return <main className="min-h-screen overflow-x-hidden bg-[#071622] text-slate-100">
    ...
    <section className="relative mx-auto max-w-7xl px-5 pb-14 pt-8 lg:px-8"><div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_330px]"><div>
      ...
      <div className="mb-4 grid grid-cols-3 overflow-hidden rounded-xl border border-sky-100/10 bg-[#0b2030]"><Metric label="Selected" value={`Week ${week}`} /><Metric label="On deck" value={forecast ? `${forecast.games.length} games` : 'Loading'} /><Metric label="Applied" value={`${appliedCount} update${appliedCount === 1 ? '' : 's'}`} /></div>
      {forecastError && <Notice tone="error" text={forecastError} />}{liveError && <Notice tone="warn" text={`${liveError} Baseline forecasts remain visible.`} />}{!forecast && !forecastError && <LoadingCards />}
...
function Metric({ label, value }: { label: string; value: string }) { return <div className="px-4 py-3 first:border-r first:border-sky-100/10 [&:nth-child(2)]:border-r [&:nth-child(2)]:border-sky-100/10"><p className="text-[10px] font-bold uppercase tracking-[.15em] text-slate-500">{label}</p><p className="mt-1 text-sm font-bold text-white">{value}</p></div>; }
