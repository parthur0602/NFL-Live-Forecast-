...
function fitAndScore(rows, bundle, config) {
  ...
  for (let start = 0; start < rows.length;) {
    ...
    for (const row of batch) {
      ...
      const record = { ...row, challengerProbability: sigmoid(logit(row.marketProbability) + config.scale * correction) };
      result.push(record);
    }
